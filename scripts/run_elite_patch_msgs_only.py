...
# -*- coding: utf-8 -*-
"""
run_elite_patch_msgs_only.py — Upload messages only (no compliment_detail).
Run this first, before the SQL migration is applied.
After running add_compliment_detail_column.sql, run run_elite_patch_compliments.py.
"""
...
def patch(lead_id, msg):
    body = json.dumps({'custom_message': msg, 'status': 'READY TO SEND'},
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  Elite Patch — MESSAGES ONLY  |  {"DRY-RUN" if args.dry_run else "LIVE"}')
